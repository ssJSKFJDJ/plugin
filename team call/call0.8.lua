...
function team_add(msg) --添加个人
    ...
    local k = tonumber(adder) --将adder这个字符串转成数字以比较
    if (k == nil or k <= 4) then
...
function team_del(msg) --删除个人
    local deler = string.match(msg.fromMsg, "%d+") --同上
    local Gid = "G" .. msg.fromGroup --同上
    local k = tonumber(deler)
    if (deler == nil or deler <= 4) then
        return "请使用.team del+@调查员 来将调查员移出本群team" --同上
    elseif (deler >= 13) then
        return "请保证每次只移出一名调查员" --同上
    else
        local if_have_team = index_search(call_list[Gid]) --同上
